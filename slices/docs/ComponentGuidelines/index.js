    ...
    <div className={"flex xl:gap-12 gap-8 " + singleItemClassName}>
      ...
      { 
        slice?.items?.map((item, i) => 
          <div className="flex flex-col bg-white flex-1 rounded-2xl overflow-hidden" key={i}>
          
            
            <img src={item.image.url} alt={item.image.alt} className="self-center" />
            
            <div className={ [item.type === "positive" ? "border-green-700 bg-green-100" : "border-red-700 bg-red-100", "border-t-8", "p-6", "grow"].join(" ") }>
